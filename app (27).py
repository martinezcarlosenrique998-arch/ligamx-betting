...
def get_live_odds(api_key, sport_key, markets="h2h,totals"):
    """Fetcha h2h + totals en el endpoint /odds (featured markets).
    
    NOTA: btts es un 'additional market' que requiere /events/{id}/odds
    y solo puede pedirse partido por partido. NO mezclar en esta llamada.
    """
...
def get_apifootball_lineups(api_key, fixture_id):
    """Fetcha alineaciones de API-Football (RapidAPI, plan free 100 req/día).
    Retorna dict con home/away lineups o None si no disponible."""
    ...
    try:
        r = requests.get(url, headers=headers, params={"fixture": fixture_id}, timeout=8)
        if r.status_code == 200:
            data = r.json()
            lineups = data.get("response", [])
            if len(lineups) >= 2:
                return {"home": lineups[0], "away": lineups[1], "source": "api-football"}
    except Exception:
        pass
    return None
...
def parse_lineup_strengths(lineup_resp):
    """Analiza un lineup de API-Football y detecta posiciones clave ausentes.
    Retorna dict con startXI, formation, y flags de jugadores clave."""
    if not lineup_resp:
        return {}
    result = {}
    for team_data in lineup_resp if isinstance(lineup_resp, list) else [lineup_resp]:
        team_name = team_data.get("team", {}).get("name", "")
        start_xi  = [p["player"]["name"] for p in team_data.get("startXI", [])]
        formation = team_data.get("formation", "?")
        result[team_name] = {"xi": start_xi, "formation": formation, "n": len(start_xi)}
    return result
...
4. Clic en **Save** — listo, no volverás a ingresarlas
    ...
    _sec_apifb  = st.secrets.get("APIFOOTBALL_KEY",  "") if hasattr(st, "secrets") else ""
    ...
    apifootball_key  = _sec_apifb  or st.text_input("API-Football Key (RapidAPI)", type="password",
                           help="100 req/día gratis · rapidapi.com — alineaciones automáticas")
    ...
    st.caption("v5.2 · football-data.org · The Odds API · Claude AI")
...
3. Recibirás el token en tu correo
    ...
    if "pred" in st.session_state and "auto_data" in st.session_state:
        ...
        # ── Checklist de Alineaciones + Ajuste Kelly ──
        st.markdown("---")
        st.markdown("### 👥 Alineaciones & Contexto")
        st.caption("Verifica antes de apostar. El Kelly se ajusta automáticamente según las ausencias.")
        ...
        with _lu_e1:
            lu_nv  = st.checkbox("⚠️ No he verificado alineaciones", value=False, key="lu_nv")
        with _lu_e2:
            lu_rot = st.checkbox("🔄 Riesgo de rotación (copa/fatiga)", value=False, key="lu_rot")
        ...
        _lineup_data = {
            ...
            "away_gk":       lu_ag,
            "not_verified":  lu_nv,
            "rotation_risk": lu_rot,
        }
        _lu_factor, _lu_desc, _lu_icon = lineup_kelly_factor(_lineup_data)
...
with tab5:
    ...
    st.markdown("Analiza automáticamente **todos los partidos** de la jornada y detecta cuáles tienen valor esperado positivo sin ir uno por uno.")
    ...
    if not football_api_key or not odds_api_key:
        ...
        with sc_col1:
            sc_min_ev = st.slider("EV mínimo para incluir (%)", 3, 25, 7, key="sc_ev")
        with sc_col2:
        ...
        st.info("💡 El scanner usa **1 request de The Odds API** para obtener todos los partidos + momios, luego corre el modelo en cada uno. BTTS requiere 1 request adicional por partido (cuida el plan gratuito de 500 req/mes).")

        if st.button("🚀 Escanear jornada completa", type="primary", use_container_width=True, key="sc_run"):
            with st.spinner("Obteniendo partidos y momios..."):
                # 1. Obtener todos los partidos con momios h2h+totals
                sc_live = get_live_odds(odds_api_key, ODDS_SPORT, "h2h,totals")

            if isinstance(sc_live, dict) and "error_code" in sc_live:
                st.error(f"Error The Odds API: {sc_live.get('message','')}")
            elif not sc_live:
                st.warning("No hay partidos disponibles en los próximos días para esta liga.")
            else:
                st.caption(f"✅ {len(sc_live)} partidos encontrados con momios")

                # 2. Obtener partidos del calendario football-data.org
                with st.spinner("Obteniendo calendario..."):
                    upcoming = get_upcoming_matches(football_api_key, COMP_CODE)
                ...
                for idx, match in enumerate(upcoming[:20]):  # max 20 partidos
                    ...
                    progress_bar.progress((idx + 1) / max(len(upcoming[:20]), 1))
                    status_text.text(f"Analizando: {home_team} vs {away_team}...")

                    # Buscar en live_data para matchear odds
                    _event_id = find_event_id(sc_live, home_team, away_team)
                    if not _event_id:
                        sc_skipped += 1
                        continue

                    # Extraer momios h2h
                    h2h_odds    = best_odds_for_market(sc_live, home_team, away_team, "h2h")
                    totals_odds = best_odds_for_market(sc_live, home_team, away_team, "totals")
                    btts_odds   = {}

                    # BTTS solo si el usuario lo pidió (cuesta 1 req extra)
                    if "BTTS" in sc_markets:
                        btts_odds = get_btts_odds_for_event(odds_api_key, ODDS_SPORT, _event_id)

                    if not h2h_odds and not totals_odds:
                        sc_skipped += 1
                        continue

                    # Correr el modelo predictivo
                    try:
                        home_id = match.get("homeTeam", {}).get("id")
                        away_id = match.get("awayTeam", {}).get("id")

                        # Datos mínimos: forma reciente
                        _h_matches = get_team_matches(football_api_key, home_id, last=8) if home_id else []
                        _a_matches = get_team_matches(football_api_key, away_id, last=8) if away_id else []

                        if len(_h_matches) < 4 or len(_a_matches) < 4:
                            sc_skipped += 1
                            continue

                        # Calcular predicciones
                        pred = calc_all_predictions(
                            home_matches=_h_matches,
                            away_matches=_a_matches,
                            h2h_matches=[],
                            home_name=home_team,
                            away_name=away_team,
                            league_code=COMP_CODE,
                        ...
                        if "1X2" in sc_markets and h2h_odds:
                            for outcome_key, my_prob, label in [
                                (home_team, pred["home_win"], f"🏠 {home_team}"),
                                ("Draw",    pred["draw"],     "🤝 Empate"),
                                (away_team, pred["away_win"], f"✈️ {away_team}"),
                            ]:
                                # Buscar momio que corresponde
                                mkt_price = None
                                for k, v in h2h_odds.items():
                                    if outcome_key.lower()[:6] in k.lower() or k.lower()[:6] in outcome_key.lower():
                                        mkt_price = v; break
                                if mkt_price is None:
                                    if outcome_key == "Draw":
                                        mkt_price = h2h_odds.get("Draw")
                                if mkt_price:
                                    dec = american_to_decimal(mkt_price)
                                    ev  = calc_ev(my_prob * 100, dec)
                                    if ev >= sc_min_ev / 100:
                                        k_adj, _, _ = smart_kelly(my_prob * 100, dec, "1X2", ev, max_kelly_pct)
                                        match_bets.append({
                                            "Partido": f"{home_team} vs {away_team}",
                                            "Fecha": utc_date,
                                            "Mercado": f"1X2 — {label}",
                                            "Modelo%": round(my_prob * 100, 1),
                                            "Momio": f"+{int(mkt_price)}" if mkt_price > 0 else str(int(mkt_price)),
                                            "EV%": round(ev * 100, 1),
                                            "Kelly%": k_adj,
                                            "Stake$": round(bankroll * k_adj / 100, 2),
                                        })

                        # ── Over 2.5 ──
                        if "Over 2.5" in sc_markets and totals_odds:
                            over_price = totals_odds.get("Over_2.5") or totals_odds.get("Over_2.5 ")
                            if over_price:
                                my_over = pred.get("over_25", 0.5)
                                dec     = american_to_decimal(over_price)
                                ev      = calc_ev(my_over * 100, dec)
                                if ev >= sc_min_ev / 100:
                                    k_adj, _, _ = smart_kelly(my_over * 100, dec, "Over/Under", ev, max_kelly_pct)
                                    match_bets.append({
                                        "Partido": f"{home_team} vs {away_team}",
                                        "Fecha": utc_date,
                                        "Mercado": "📈 Over 2.5",
                                        "Modelo%": round(my_over * 100, 1),
                        ...
                        if "BTTS" in sc_markets and btts_odds:
                            ...
                            if btts_price:
                                my_btts = pred.get("btts_yes", 0.5)
                                dec     = american_to_decimal(btts_price)
                                ev      = calc_ev(my_btts * 100, dec)
                                if ev >= sc_min_ev / 100:
                                    k_adj, _, _ = smart_kelly(my_btts * 100, dec, "BTTS", ev, max_kelly_pct)
                                    match_bets.append({
                                        "Partido": f"{home_team} vs {away_team}",
                                        "Fecha": utc_date,
                                        "Mercado": "⚽ BTTS Sí",
                                        "Modelo%": round(my_btts * 100, 1),
                ...
                if sc_results:
                    sc_df = (
                        __import__("pandas").DataFrame(sc_results)
                        .sort_values("EV%", ascending=False)
                        .reset_index(drop=True)
                    )
                    total_stake = sc_df["Stake$"].sum()
                    n_bets = len(sc_df)

                    r1, r2, r3 = st.columns(3)
                    r1.metric("Apuestas con valor", n_bets)
                    r2.metric(f"EV máximo", f"{sc_df['EV%'].max():.1f}%")
                    r3.metric("Stake total sugerido", f"${total_stake:,.0f}")

                    st.dataframe(
                        sc_df.style
                            .background_gradient(subset=["EV%"], cmap="RdYlGn")
                            .format({"EV%": "{:.1f}%", "Modelo%": "{:.1f}%", "Stake$": "${:.2f}"}),
                        use_container_width=True,
                        hide_index=True
                    )

                    if sc_skipped:
                        st.caption(f"ℹ️ {sc_skipped} partidos omitidos (sin datos suficientes o sin match de odds)")

                    # Botón Telegram — enviar todas las alertas
                    if telegram_token and telegram_chat_id:
                        if st.button("📨 Enviar TODAS a Telegram", type="secondary", key="sc_tg"):
                            _sent = 0
                            for _, row in sc_df.iterrows():
                                try:
                                    _am = float(row["Momio"].replace("+",""))
                                except Exception:
                                    _am = -110.0
                                msg = format_bet_alert(
                                    partido=row["Partido"], liga=COMP_CODE,
                                    mercado=row["Mercado"],
                                    prob_modelo=row["Modelo%"],
                                    momio_am=_am,
                                    ev_pct=row["EV%"],
                                    kelly_pct=row["Kelly%"],
                                    stake=row["Stake$"],
                    ...
                    _total = len(upcoming[:20]) - sc_skipped
                    st.success(f"✅ {_total} partidos analizados — ninguno supera el umbral de EV ≥ {sc_min_ev}%")
                    st.info("Prueba bajar el EV mínimo o ampliar a más días para ver más candidatos.")
                    if sc_skipped:
                        st.caption(f"ℹ️ {sc_skipped} partidos omitidos por falta de datos o sin match de odds")
