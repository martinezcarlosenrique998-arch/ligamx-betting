    ...
    for s in seasons_to_try:
        for variant in variants:
            try:
                ...
                for m in team_data:
                    try:
                        is_home = m.get("h", {}).get("title") == variant or \
                                  m.get("side") == "h"
                        # get_match_data retorna por partido, no por equipo
                        # Detectar si el equipo es local o visitante
                        h_title = m.get("h", {}).get("title", "")
                        a_title = m.get("a", {}).get("title", "")
                        is_home = (h_title == variant or
                                   h_title.replace("_"," ") == variant.replace("_"," "))
                        if is_home:
