...
def get_live_odds(api_key, sport_key, markets="h2h,totals,btts"):
    """Fetcha h2h + totals + btts en una sola llamada (1 request del plan gratuito)."""
            ...
            if isinstance(live_data, dict) and live_data.get("error_code"):
                ...
                if "home_team" in st.session_state:
                    ...
                    if matched:
                        ...
                        btts_odds   = best_odds_for_market(live_data, _ht, _at, "btts")
                        if totals_odds:
                        ...
                        if totals_odds:
                            ...
                            st.caption(f"📊 Over 2.5: {'+' if isinstance(o25,int) and o25>0 else ''}{o25} · Under 2.5: {'+' if isinstance(u25,int) and u25>0 else ''}{u25}")
                        if btts_odds:
                            by = btts_odds.get("yes","—"); bn = btts_odds.get("no","—")
                            st.caption(f"📊 BTTS Sí: {'+' if isinstance(by,int) and by>0 else ''}{by} · No: {'+' if isinstance(bn,int) and bn>0 else ''}{bn}")
