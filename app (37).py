...
def get_team_xg_from_fdorg(api_key, team_id, team_name, n_matches=14):
    ...
    try:
        ...
        for m in matches:
            ...
            # Proxy xG: goles * factor de regresión a la media
            # (calibrado: goles reales sobreestiman ligeramente el xG)
            if xg_team is None:
                g_team = hg if is_home else ag
                g_opp  = ag if is_home else hg
                xg_team = round(g_team * 0.85 + 0.15, 3)  # regresión a ~1.2
                xg_opp  = round(g_opp  * 0.85 + 0.15, 3)
