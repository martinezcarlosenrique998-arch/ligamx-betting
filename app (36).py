...
def get_live_odds(api_key, sport_key, markets="h2h,totals"):
    ...
    # Intento 1: solo casas MX conocidas (ahorra requests y filtra ruido)
    params = {
        "apiKey":      api_key,
    ...
    # Fallback: regiones eu+uk completas (puede incluir casas no-MX, se filtra en UI)
    params2 = {
        "apiKey":     api_key,
        "regions":    MX_REGIONS,
        "markets":    markets,
    ...
    try:
        r2   = requests.get(url, params=params2, timeout=12)
        data2 = r2.json()
        if r2.status_code != 200:
            return {"error_code": r2.status_code, "message": data2.get("message", str(data2))}
        return data2
    except Exception as e:
...
def get_understat_xg(team_name, understat_league, season=None):
    """
    Obtiene xG real desde understat.com usando requests directo.
    Sin understatapi — sin asyncio — sin deadlocks.
    URL: https://understat.com/team/{name}/{season}
    Los datos están en un <script> tag como JSON unicode-escaped.
    """
    if not understat_league:
        return []

    import re, json as _json, codecs
    ...
    def name_variants(name):
        ...
        for v in [clean.replace(" ","_"), strip_accents(clean).replace(" ","_"), name.replace(" ","_")]:
    ...
    for s in seasons_to_try:
        for variant in variants:
            try:
                url = f"https://understat.com/team/{variant}/{s}"
                r   = requests.get(url, headers=headers, timeout=12)
                if r.status_code == 404:
                    errors.append(f"{variant}/{s}: 404")
                    continue
                if r.status_code != 200:
                    errors.append(f"{variant}/{s}: HTTP {r.status_code}")
                    continue

                # Extraer JSON del script tag
                # understat guarda los datos así:
                # var datesData = JSON.parse('\x5b...\x5d')
                # Intentar ambos formatos de quotes que usa understat
                match = re.search(
                    r"var\s+datesData\s*=\s*JSON\.parse\(['\"]([^'\"]+)['\"]\)",
                    r.text
                )
                if not match:
                    # Último intento: buscar cualquier variable que contenga datos de partidos
                    match = re.search(
                        r"var\s+\w*[Dd]ates?[Dd]ata\w*\s*=\s*JSON\.parse\(['\"]([^'\"]{50,})['\"]\)",
                        r.text
                    )
                if not match:
                    # Guardar preview del HTML para diagnóstico
                    html_vars = re.findall(r"var\s+(\w+)\s*=\s*JSON\.parse\(", r.text)
                    errors.append(f"{variant}/{s}: datesData no encontrado. Vars JSON disponibles: {html_vars}. HTML len={len(r.text)}")
                    continue

                # Decodificar unicode escapes (understat usa \xNN hex)
                raw_str = match.group(1)
                try:
                    # Método 1: raw_unicode_escape (más común en understat)
                    decoded = raw_str.encode('raw_unicode_escape').decode('unicode_escape')
                    team_data = _json.loads(decoded)
                except Exception:
                    try:
                        # Método 2: decodificar \xNN con ast.literal_eval
                        import ast as _ast
                        decoded2 = _ast.literal_eval(f'"{raw_str}"')
                        team_data = _json.loads(decoded2)
                    except Exception:
                        try:
                            # Método 3: reemplazar \xNN directamente con regex
                            def _unescape(s):
                                return re.sub(r'\\x([0-9a-fA-F]{2})',
                                             lambda m: chr(int(m.group(1),16)), s)
                            decoded3 = _unescape(raw_str)
                            team_data = _json.loads(decoded3)
                        except Exception as je:
                            errors.append(f"{variant}/{s}: JSON parse error (3 métodos): {str(je)[:60]}")
                            continue
                ...
                for m in team_data:
                    try:
                        date_str = m.get("datetime", m.get("date", ""))[:10]
                        # Saltar partidos futuros
                        if date_str > today_str:
                            continue
                        # Determinar si es local o visitante
                        is_home = m.get("h_a", m.get("side", "")) == "h"
                        if is_home:
                ...
                if len(matches) >= 5:
                    ...
                    errors.append(f"{variant}/{s}: solo {len(matches)} partidos jugados")

            except requests.exceptions.Timeout:
                errors.append(f"{variant}/{s}: timeout HTTP")
                continue
            except Exception as e:
                errors.append(f"{variant}/{s}: {type(e).__name__}: {str(e)[:60]}")
                continue
...
APISPORTS_DEFAULT_KEY = "900977316fa2c911a8340f02cd40ce99"
...
def apisports_get_lineups(api_key, fixture_id):
    ...
    for team_data in resp:
        ...
        # lineup_confirmed solo aparece en algunos fixtures
        confirmed = len(start_xi) == 11

        side = "home" if result["home"] is None else "away"
        result[side] = {
            "team":      team_name,
            "formation": formation,
            "xi":        start_xi,
...
def classify_injury_impact(injuries, home_team_name, away_team_name):
    ...
    results = {}
    for side, team_lower in [("home", home_lower), ("away", away_lower)]:
        side_injuries = [i for i in injuries if team_lower[:8] in i.get("team", "").lower()]
...
4. Clic en **Save** — listo, no volverás a ingresarlas
    ...
    _sec_apifb  = st.secrets.get("APISPORTS_KEY", "900977316fa2c911a8340f02cd40ce99") if hasattr(st, "secrets") else "900977316fa2c911a8340f02cd40ce99"
    ...
    if home_team_id and away_team_id:
        ...
        if st.button("🔮 ANALIZAR AUTOMÁTICAMENTE", use_container_width=True, type="primary"):
            ...
            if understat_league and UNDERSTAT_AVAILABLE:
                with st.spinner("Cargando xG reales de Understat..."):
                ...
                if home_errs:
                    xg_debug["home_errors"] = home_errs[:3]
                    # Mostrar primer error para diagnóstico visible
                    if home_errs:
                        st.caption(f"⚠️ xG local: {home_errs[0][:120]}")
                if away_errs:
                    xg_debug["away_errors"] = away_errs[:3]
                    if away_errs:
                        st.caption(f"⚠️ xG visitante: {away_errs[0][:120]}")
...
with tab3:
    ...
    _default_am  = float(_pf.get("momio",   _pf.get("american", 240.0)))
    _default_p   = float(_pf.get("my_prob", 28.0))
                ...
                for liga_idx, liga_name in enumerate(sc_selected_leagues):
                    ...
                    for m_idx, match in enumerate(upcoming[:15]):
                        ...
                        if "Over 2.5" in sc_markets and totals_odds:
                            for key_o in ("Over_2.5", "Over_2.50", "Over 2.5"):
                                ...
                                if over_price:
                                    break
                            if over_price:
                                my_over = pred.get("over_25", pred.get("over25", 0.5))
                                dec     = american_to_decimal(over_price)
                                ev      = calc_ev(my_over * 100, dec)
                                if ev * 100 >= sc_min_ev:
                                    k, _, riesgo = smart_kelly(my_over * 100, dec, "Over/Under", ev, max_kelly_pct)
                        ...
                        if "BTTS" in sc_markets and btts_odds:
                            ...
                            if btts_price:
                                my_btts = pred.get("btts_yes", pred.get("btts", 0.5))
                                dec     = american_to_decimal(btts_price)
                                ev      = calc_ev(my_btts * 100, dec)
                                if ev * 100 >= sc_min_ev:
                                    k, _, riesgo = smart_kelly(my_btts * 100, dec, "BTTS", ev, max_kelly_pct)
                                    k = round(k * _sc_context_factor, 2)
            ...
            with cc5:
                if st.button("→ Calc EV", key=f"sc_calc2_{sc_idx}", help="Enviar a Calculadora EV"):
                    ...
                    st.session_state["calc_prefill"] = {
                        ...
                        "mercado":  row["Mercado"],
                        "momio":    _mom_raw,
                        "my_prob":  float(row.get("_my_prob", row.get("Modelo%", 50))),
