...
def get_understat_xg(team_name, understat_league, season=None):
    ...
    def _fetch_understat(variant, s):
        """Llama a UnderstatClient con timeout — se cuelga frecuentemente."""
        from concurrent.futures import ThreadPoolExecutor, TimeoutError as FuturesTimeout
        def _inner():
            with UnderstatClient() as understat:
                return understat.team(team=variant).get_match_data(season=str(s))
    ...
    for s in seasons_to_try:
        for variant in variants:
            try:
                team_data = _fetch_understat(variant, s)
                if team_data is None:
                    errors.append(f"{variant}/{s}: timeout (>8s)")
                    continue
    ...
    if home_team_id and away_team_id:
        ...
        if st.button("🔮 ANALIZAR AUTOMÁTICAMENTE", use_container_width=True, type="primary"):
            ...
            if understat_league and UNDERSTAT_AVAILABLE:
                with st.spinner("Jalando xG de Understat... (máx 20s, continúa sin xG si tarda)"):
                    from concurrent.futures import ThreadPoolExecutor, TimeoutError as _FT
                    def _get_both():
                        h = get_understat_xg(home_team_name, understat_league)
                        a = get_understat_xg(away_team_name, understat_league)
                        return h, a
                    with ThreadPoolExecutor(max_workers=1) as _ex:
                        _fut = _ex.submit(_get_both)
                        try:
                            home_understat, away_understat = _fut.result(timeout=20)
                        except (_FT, Exception):
                            home_understat, away_understat = [], []
                            st.caption("⚠️ Understat no respondió — usando estadísticas de liga.")
