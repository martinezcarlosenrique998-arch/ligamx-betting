    ...
    if not football_api_key or not odds_api_key:
        ...
        if not sc_selected_leagues:
            ...
            if st.button("🚀 ESCANEAR TODAS LAS LIGAS", type="primary", use_container_width=True, key="sc_run"):
                ...
                if sc_all_results:
                    sc_df = pd.DataFrame(sc_all_results).sort_values("EV%", ascending=False).reset_index(drop=True)
                    ...
                    st.dataframe(
                        sc_df[["Liga","Partido","Fecha","Mercado","Modelo%","Momio","EV%","Kelly%","Stake$","Riesgo"]]
                        .style.background_gradient(subset=["EV%"], cmap="RdYlGn")
                        .format({"EV%": "{:.1f}%", "Modelo%": "{:.1f}%", "Stake$": "${:.2f}"}),
                        use_container_width=True, hide_index=True
                    )

                    if sc_total_skipped:
                        st.caption(f"ℹ️ {sc_total_skipped} partidos omitidos (sin datos suficientes)")

                    # Guardar en session_state para paper trading
                    st.session_state["sc_results"] = sc_df.to_dict("records")
                    ...
                    if telegram_token and telegram_chat_id:
                        if st.button("📨 Enviar TODO a Telegram", key="sc_tg", type="secondary"):
                            ...
                            for _, row in sc_df.iterrows():
                                try:
                                    _am = float(str(row["Momio"]).replace("+", ""))
                                except Exception:
                                    _am = -110.0
                                msg = format_bet_alert(
                                    partido=row["Partido"], liga=row["Liga"],
                                    mercado=row["Mercado"], prob_modelo=row["Modelo%"],
                    ...
                    st.success(f"✅ Escaneo completo — ningún partido supera EV ≥ {sc_min_ev}%")
                    st.info("Prueba bajar el EV mínimo o ampliar el rango de días.")
