...
try:
    from context_pipeline import run_context_pipeline, USE_CONTEXT as PIPELINE_USE_CONTEXT
    _CONTEXT_PIPELINE_AVAILABLE = True
except ImportError:
    _CONTEXT_PIPELINE_AVAILABLE = False
    PIPELINE_USE_CONTEXT = False

    def run_context_pipeline(*_a, **_k):
        return {
            "used_context": False,
            "base_prediction": {},
            "adjusted_prediction": {},
            "features": {},
            "text_bundle": {},
            "error": "context_pipeline import unavailable",
        }
...
with st.sidebar:
    ...
    use_context_pipeline = st.checkbox(
        "Usar pipeline contextual (noticias/alineaciones/contexto)",
        value=bool(PIPELINE_USE_CONTEXT),
        help="Aplica un ajuste incremental y acotado sobre el modelo base. Si falla, usa el baseline.",
    )
...
with tab1:
        ...
        if not matches:
            ...
            if st.button("🔮 ANALIZAR PARTIDO", type="primary", use_container_width=True):
                with st.spinner("Descargando historial y calculando Poisson..."):
                    ...
                    pred_base = calc_all_predictions(
                        h_matches, a_matches, home_team_id, away_team_id, LEAGUE_AVG, COMP_CODE
                    )
                    context_payload = run_context_pipeline(
                        home_team_name,
                        away_team_name,
                        pred_base,
                        anthropic_key=anthropic_key,
                        use_context=(use_context_pipeline and _CONTEXT_PIPELINE_AVAILABLE),
                    )
                    pred = context_payload.get("adjusted_prediction") or pred_base
    ...
    if "pred" in st.session_state:
        ...
        context_payload = st.session_state.get("context_payload", {})
        if context_payload.get("used_context"):
            base_pred = st.session_state.get("pred_base", {})
            features = context_payload.get("features", {})
            st.markdown("#### 🧠 Ajuste Contextual")
            cc1, cc2 = st.columns(2)
            cc1.metric(
                "Δ Local",
                f"{pred['home_win_pct'] - float(base_pred.get('home_win_pct', pred['home_win_pct'])):+.1f} pts",
            )
            cc2.metric(
                "Δ Visita",
                f"{pred['away_win_pct'] - float(base_pred.get('away_win_pct', pred['away_win_pct'])):+.1f} pts",
            )
            with st.expander("Ver features contextuales", expanded=False):
                st.json(features)
                text_bundle = context_payload.get("text_bundle", {})
                st.caption(f"Noticias extraídas: {len(text_bundle.get('news_items', []))}")
                if context_payload.get("error"):
                    st.caption(f"Fallback usado: {context_payload['error']}")
    ...
    if not football_api_key or not _get_odds_keys_pool():
        ...
        if st.button("🚀 Iniciar Escaneo V8", type="primary"):
            ...
            for lname in sc_leagues:
                ...
                # Cache por corrida para evitar scraping/LLM repetido por partido
                ctx_cache = st.session_state.setdefault("_scanner_context_cache", {})
                ...
                for m in matches[:5]:
                    ...
                    pred_base = calc_all_predictions(h_matches, a_matches, h_id, a_id, l_avg, cfg["code"])
                    pred = pred_base
                    ctx_key = f"{cfg['code']}|{m.get('id')}|{h_name} vs {a_name}"
                    if use_context_pipeline and _CONTEXT_PIPELINE_AVAILABLE:
                        cached = ctx_cache.get(ctx_key)
                        if not cached:
                            cached = run_context_pipeline(
                                h_name,
                                a_name,
                                pred_base,
                                anthropic_key=anthropic_key,
                            ...
                            ctx_cache[ctx_key] = cached
                        pred = cached.get("adjusted_prediction") or pred_base
                    ...
                    if o25_dec and u25_dec:
                        ...
                        if vmin <= div_dyn <= vmax:
                            ...
                            results.append({
                                ...
                                "Ctx": "Sí" if (use_context_pipeline and _CONTEXT_PIPELINE_AVAILABLE) else "No",
