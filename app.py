...
@st.cache_data(ttl=86400)  # cachear lista de equipos 24h
def _get_understat_league_teams(understat_league, season):
    """Obtiene la lista de equipos de la liga en Understat para esa temporada."""
    if not UNDERSTAT_AVAILABLE:
        return {}
    try:
        client = UnderstatClient()
        league_data = client.league(league=understat_league).get_team_data(season=str(season))
        # Retorna dict: {team_name: {...stats...}}
        return {name: True for name in league_data.keys()}
    except Exception:
        return {}
...
def get_understat_xg(team_name, understat_league, season=None):
    """
    Obtiene xG real desde Understat usando fuzzy matching automático.
    No requiere mapeo manual de nombres — siempre encuentra el equipo correcto.
    """
    ...
    for s in seasons_to_try:
        try:
            ...
            league_teams = _get_understat_league_teams(understat_league, s)
            if not league_teams:
                continue

            # 2. Encontrar el mejor match por fuzzy score
            best_name  = None
            best_score = 0
            for ut_name in league_teams:
                score = _fuzzy_score(team_name, ut_name)
                if score > best_score:
                    best_score, best_name = score, ut_name

            # Umbral mínimo de 40 para evitar falsos positivos
            if best_score < 40 or best_name is None:
                continue

            # 3. Obtener stats del equipo encontrado
            client = UnderstatClient()
            team_data = client.team(team=best_name).get_team_stats(season=str(s))
            if not team_data:
                continue
        ...
        if st.button("🔮 ANALIZAR AUTOMÁTICAMENTE", use_container_width=True, type="primary"):
            ...
            if understat_league and UNDERSTAT_AVAILABLE:
                ...
                if home_understat:
                    ...
                    st.caption(f"⚠️ xG no encontrado para: `{home_team_name}` en Understat ({understat_league})")
                if away_understat:
                    _, _, away_xgf, away_xga, _, _ = calc_xg_averages(away_understat)
                    away_xg_reg = calc_xg_overperformance(away_understat)
                else:
                    st.caption(f"⚠️ xG no encontrado para: `{away_team_name}` en Understat ({understat_league})")
            elif not UNDERSTAT_AVAILABLE:
                st.caption("⚠️ understatapi no instalado — xG desactivado")
