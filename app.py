...
def get_understat_xg(team_name, understat_league, season=None):
    ...
    Busca directamente por nombre con múltiples variantes — sin depender de get_team_data.
    """
    ...
    # Generar variantes del nombre automáticamente
    def name_variants(name):
        variants = [name]
        # Quitar sufijos
        for suffix in [" FC", " CF", " AC", " AFC", " BC", " SC", " SL",
                       " Balompié", " Balompie", " de Fútbol", " de Futbol"]:
            cleaned = name.replace(suffix, "").strip()
            if cleaned not in variants: variants.append(cleaned)
        # Quitar prefijos
        for prefix in ["FC ", "CF ", "AC ", "RC ", "RCD ", "UD ", "CD ", "CA ",
                       "SD ", "AS ", "SS ", "US ", "SC ", "OGC ", "LOSC "]:
            if name.startswith(prefix):
                stripped = name[len(prefix):].strip()
                if stripped not in variants: variants.append(stripped)
        # Versión sin acentos
        import unicodedata
        def strip_accents(s):
        ...
        for v in list(variants):
            sa = strip_accents(v)
            if sa not in variants: variants.append(sa)
        # Variantes específicas conocidas
        known = {
            "Valencia CF":              ["Valencia"],
            "Deportivo Alavés":         ["Alaves", "Deportivo Alaves"],
            "Club Atlético de Madrid":  ["Atletico Madrid", "Atletico de Madrid"],
            "Real Betis Balompié":      ["Real Betis Balompie", "Real Betis"],
            "Real Sociedad de Fútbol":  ["Real Sociedad"],
            "Rayo Vallecano de Madrid": ["Rayo Vallecano"],
            "RC Celta de Vigo":         ["Celta Vigo", "Celta de Vigo"],
            ...
            "FC Internazionale Milano": ["Internazionale", "Inter"],
            "FC Bayern München":        ["Bayern Munich", "Bayern"],
            "Borussia Mönchengladbach": ["Borussia M.Gladbach", "M'gladbach"],
        ...
        for extra in known.get(name, []):
            if extra not in variants: variants.append(extra)
        return variants
    ...
    for s in seasons_to_try:
        for variant in variants:
            try:
                ...
                team_data = client.team(team=variant).get_team_stats(season=str(s))
                ...
                if len(matches) >= 5:
                    ...
                    errors.append(f"{variant}/{s}: solo {len(matches)} partidos")
            except Exception as e:
                errors.append(f"{variant}/{s}: {type(e).__name__}: {str(e)[:60]}")
                continue
    # Retornar errores como item especial para debug
    ...
    if home_team_id and away_team_id:
        ...
        if st.button("🔮 ANALIZAR AUTOMÁTICAMENTE", use_container_width=True, type="primary"):
            ...
            if understat_league and UNDERSTAT_AVAILABLE:
                ...
                xg_debug["league"]       = understat_league
                ...
                if home_errs: xg_debug["home_errors"] = home_errs[:5]
                if away_errs: xg_debug["away_errors"] = away_errs[:5]
                xg_debug["home_sample"]  = next((m for m in home_understat if "_errors" not in m), None)
                xg_debug["away_sample"]  = next((m for m in away_understat if "_errors" not in m), None)
            ...
            with st.expander("🔍 Debug xG (temporal)", expanded=False):
                st.json(xg_debug)
