...
UNDERSTAT_NAME_MAP = {
    # Premier League — football-data.org nombre exacto → understat nombre exacto
    "Arsenal FC":                  "Arsenal",
    "Chelsea FC":                  "Chelsea",
...
def get_understat_xg(team_name, understat_league, season=2024):
    """
    Obtiene xG real desde Understat. Intenta múltiples variantes del nombre.
    """
    if not UNDERSTAT_AVAILABLE or not understat_league:
        return []

    # Construir lista de nombres a intentar
    names_to_try = []
    if team_name in UNDERSTAT_NAME_MAP:
        names_to_try.append(UNDERSTAT_NAME_MAP[team_name])
    # Fallbacks: quitar sufijos comunes
    clean = (team_name
             .replace(" FC", "").replace(" CF", "").replace(" AC", "")
             .replace(" AFC", "").replace(" BC", "").replace(" SC", "")
             .strip())
    if clean not in names_to_try:
        names_to_try.append(clean)
    if team_name not in names_to_try:
        names_to_try.append(team_name)

    for name in names_to_try:
        try:
            client = UnderstatClient()
            team_data = client.team(team=name).get_team_stats(season=str(season))
            if not team_data:
                continue
            matches = []
            for m in team_data:
