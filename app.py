...
STADIUM_COORDS = {
    ...
    "Atlético de Madrid":         (40.4361,  -3.5995),
    "Athletic Club":              (43.2641,  -2.9494),
    "Real Sociedad":              (43.3015,  -1.9732),
    "Villarreal CF":              (39.9444,  -0.1031),
    "Real Betis":                 (37.3561,  -5.9820),
...
@st.cache_data(ttl=1800)  # 30 min — clima cambia
def get_match_weather(team_name, match_date_str):
    """
    Obtiene el clima pronosticado para la hora del partido.
    Usa Open-Meteo: completamente gratis, sin API key.
    ...
    try:
        ...
        # Buscar hora ~15:00 del día del partido (hora típica de partido)
        target_prefix = match_date_str[:10] + "T15"
        ...
        if idx is None:
            # fallback: primer slot del día
            target_prefix = match_date_str[:10]
            idx = next((i for i, t in enumerate(times) if t.startswith(target_prefix)), None)
        if idx is None:
            return None
        ...
        # WMO weather codes → categoría legible
...
LEAGUE_CALIBRATION = {
    "PL":  {"avg_home": 1.53, "avg_away": 1.21, "over25_base": 0.565, "btts_base": 0.545},
    "PD":  {"avg_home": 1.42, "avg_away": 1.16, "over25_base": 0.530, "btts_base": 0.500},
    "BL1": {"avg_home": 1.68, "avg_away": 1.38, "over25_base": 0.615, "btts_base": 0.585},
    "SA":  {"avg_home": 1.38, "avg_away": 1.10, "over25_base": 0.500, "btts_base": 0.490},
    "FL1": {"avg_home": 1.35, "avg_away": 1.13, "over25_base": 0.515, "btts_base": 0.505},
    "CL":  {"avg_home": 1.70, "avg_away": 1.15, "over25_base": 0.580, "btts_base": 0.545},
}
...
def calc_all_predictions(home_form, away_form, home_gf, away_gf,
    ...
    # ── xG real de Understat: 70% xG real, 30% goles reales ──
    ...
    if elo_home_p is not None:
        elo_diff_factor = (elo_home - elo_away) / 400.0
        xg_h *= (1.0 + elo_diff_factor * 0.08)
...
3. Recibirás el token en tu correo
        ...
        if st.button("🔮 ANALIZAR AUTOMÁTICAMENTE", use_container_width=True, type="primary"):
            ...
            if match_date_str:
                with st.spinner("Jalando pronóstico del clima..."):
                    weather = get_match_weather(home_team_name, match_date_str)
                if weather:
            ...
            auto_data = {
                ...
                "weather": weather, "weather_factor": weather_factor, "weather_desc": weather_desc,
                "league_code": COMP_CODE,
            }
    ...
    if "pred" in st.session_state and "auto_data" in st.session_state:
        ...
        with src4:
            ...
            wd = data.get("weather_desc", "Sin datos")
            wx_color = "#ff4466" if wf < 0.92 else ("#ffcc44" if wf < 0.97 else "#00ff88")
            wx_label = wx["condition"] if wx else "⚠️ Sin datos"
            wx_detail = f"{wx['temp_c']}°C · {wx['wind_kmh']}km/h · {wx['precip_mm']}mm" if wx else "Estadio no mapeado"
            st.markdown(f"""<div class="metric-card">
                ...
                <div style="font-size:.72em;color:#666">Factor xG: ×{wf}</div>
            </div>""", unsafe_allow_html=True)
...
with tab2:
    ...
    for i, casa in enumerate(st.session_state["casas"]):
        ...
        # Usar text_input para momios — evita el bug de number_input con negativos
        local_str  = cols[1].text_input("", value=str(int(casa["local"])),  key=f"cl_{i}", label_visibility="collapsed", placeholder="+200")
        empate_str = cols[2].text_input("", value=str(int(casa["empate"])), key=f"ce_{i}", label_visibility="collapsed", placeholder="+230")
        visita_str = cols[3].text_input("", value=str(int(casa["visita"])), key=f"cv_{i}", label_visibility="collapsed", placeholder="-150")
        eliminar   = cols[4].button("🗑", key=f"cd_{i}")

        if not eliminar:
            try: local_v  = float(local_str)
            except: local_v = casa["local"]
            try: empate_v = float(empate_str)
            except: empate_v = casa["empate"]
            try: visita_v = float(visita_str)
            except: visita_v = casa["visita"]
