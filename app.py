...
# ─────────────────────────────────────────────
#  UNDERSTAT — xG real por equipo
# ─────────────────────────────────────────────

# Mapeo de nombres football-data.org → understat (diferencias comunes)
UNDERSTAT_NAME_MAP = {
...
def get_understat_xg(team_name, understat_league, season=2024):
    """
    Obtiene xG real de los últimos partidos de un equipo desde Understat.
    Retorna lista de dicts: [{xG, xGA, is_home, date, goals, goals_against}]
    """
    if not UNDERSTAT_AVAILABLE or not understat_league:
        return []
    try:
        mapped_name = UNDERSTAT_NAME_MAP.get(team_name, team_name)
        client = UnderstatClient()
        team_data = client.team(team=mapped_name).get_team_stats(season=str(season))
        matches = []
        for m in team_data:
            try:
                matches.append({
                    ...
                    "goals_ag":  int(m.get("missed", 0) or 0),
                    "date":      m.get("date", ""),
                    "result":    m.get("result", ""),
                })
            except:
                continue
        return matches[-12:]  # últimos 12 partidos
    except Exception:
        return []

def calc_xg_averages(understat_matches):
    """
    Calcula xG promedio como local y como visitante ponderado temporalmente.
    ...
    xgf_h = round(h_xgf / h_w, 3) if h_w > 0 else None
    xga_h = round(h_xga / h_w, 3) if h_w > 0 else None
    xgf_a = round(a_xgf / a_w, 3) if a_w > 0 else None
    xga_a = round(a_xga / a_w, 3) if a_w > 0 else None
...
@st.cache_data(ttl=86400)  # 24h — el ELO cambia poco
def get_clubelo(team_name):
    """
    Obtiene el ELO actual de un equipo desde api.clubelo.com.
    Retorna float con el ELO, o None si no se encuentra.
    """
    mapped = CLUBELO_NAME_MAP.get(team_name, team_name.replace(" ", ""))
    url = f"http://api.clubelo.com/{mapped}"
    try:
        ...
        return float(last_line[4]) if len(last_line) > 4 else None
    except:
        return None
...
def elo_win_probability(elo_home, elo_away, home_advantage=65):
    ...
    # Estimación de empate: ~26% promedio, ajustado por lo parejo del partido
    closeness = 1 - abs(p_home_nodraw - 0.5) * 2  # 1 = 50/50, 0 = dominante
    draw_prob  = 0.26 * (0.7 + closeness * 0.3)
...
def calc_all_predictions(home_form, away_form, home_gf, away_gf,
    ...
    # ── Venue split: usar datos de local/visita si están disponibles ──
    ...
    # ── Si hay xG real de Understat, reemplazar goles reales como base ──
    # xG real es más predictivo que los goles marcados (menos ruido)
    if home_xgf is not None: hgf = home_xgf * 0.70 + hgf * 0.30
    if home_xga is not None: hga = home_xga * 0.70 + hga * 0.30
    if away_xgf is not None: agf = away_xgf * 0.70 + agf * 0.30
    if away_xga is not None: aga = away_xga * 0.70 + aga * 0.30

    # ── Índices ataque/defensa brutos ──
    raw_home_att = hgf / max(LEAGUE_AVG_HOME, 0.5)
    raw_home_def = hga / max(LEAGUE_AVG_AWAY, 0.5)
    raw_away_att = agf / max(LEAGUE_AVG_AWAY, 0.5)
    raw_away_def = aga / max(LEAGUE_AVG_HOME, 0.5)
    ...
    # ── Forma ponderada ──
    home_form_mult = 1.0 + (home_form - 0.5) * 0.24
    away_form_mult = 1.0 + (away_form - 0.5) * 0.24
    ...
    # ── xG base del modelo Poisson ──
    xg_h = home_att * away_def * LEAGUE_AVG_HOME * home_form_mult * home_h2h_mult
    xg_a = away_att * home_def * LEAGUE_AVG_AWAY * away_form_mult * away_h2h_mult

    # ── Ajuste por sobreperformance xG (regresión a la media) ──
    xg_h *= home_xg_regression
    xg_a *= away_xg_regression

    # ── Ajuste por fatiga ──
    xg_h *= away_fatigue   # si el visitante está fatigado, el local se beneficia
    xg_a *= home_fatigue   # si el local está fatigado, el visitante se beneficia
    # También afecta directamente al equipo cansado
    xg_h *= (1.0 + (1.0 - home_fatigue) * 0.3)  # local fatigado anota menos
    xg_a *= (1.0 + (1.0 - away_fatigue) * 0.3)

    # ── Ajuste ELO: blend leve con probabilidades ELO ──
    elo_home_p, elo_draw_p, elo_away_p = elo_win_probability(elo_home, elo_away)
    if elo_home_p is not None:
        # Usamos ELO para ajustar xG levemente: equipos mejor rankeados reciben boost
        elo_diff_factor = (elo_home - elo_away) / 400.0  # -1 a +1 aprox
        xg_h *= (1.0 + elo_diff_factor * 0.08)  # máx ±8% ajuste por ELO
        xg_a *= (1.0 - elo_diff_factor * 0.08)

    # ── Clásico ──
    if is_clasico:
        xg_h = xg_h * 0.85 + LEAGUE_AVG_HOME * 0.15
        xg_a = xg_a * 0.85 + LEAGUE_AVG_AWAY * 0.15

    xg_h = round(max(0.25, min(4.5, xg_h)), 3)
    xg_a = round(max(0.25, min(4.5, xg_a)), 3)
    ...
    # ── Over/Under ──
    over_15  = round(matrix_sum(M, lambda i,j: i+j >= 2) * 100, 1)
    over_25  = round(matrix_sum(M, lambda i,j: i+j >= 3) * 100, 1)
    over_35  = round(matrix_sum(M, lambda i,j: i+j >= 4) * 100, 1)
    ...
    # Blend: 65% modelo, 35% historial real
    btts_yes  = round(btts_dc * 0.65 + btts_hist * 0.35, 1)
...
3. Recibirás el token en tu correo
    ...
    if home_team_id and away_team_id:
        ...
        if st.button("🔮 ANALIZAR AUTOMÁTICAMENTE", use_container_width=True, type="primary"):
            ...
            # ── Line movement: momios actuales del mercado ──
            market_h = market_d = market_a = None
            casas_actuales = st.session_state.get("casas", [])
            if casas_actuales and odds_api_key:
    ...
    if "pred" in st.session_state and "auto_data" in st.session_state:
        ...
        with src1:
            ...
            st.markdown(f"""<div class="metric-card">
                <div style="font-size:.75em;color:{xg_color}">⚽ xG Real (Understat)</div>
                <div style="font-family:'Space Mono';font-size:1.1em;color:{xg_color}">{xg_label}</div>
                <div style="font-size:.8em;color:#aaa">{ht}: xGF {hxgf} · {at}: xGF {axgf}</div>
                <div style="font-size:.75em;color:#666">Regresión: ×{data.get('home_xg_reg',1.0):.3f} / ×{data.get('away_xg_reg',1.0):.3f}</div>
        ...
        with src2:
            ...
            elo_diff  = f"{int(eh-ea):+d} pts favor {ht if eh>ea else at}" if eh and ea else ""
            st.markdown(f"""<div class="metric-card">
                <div style="font-size:.75em;color:{elo_color}">📊 ELO (ClubElo)</div>
                <div style="font-family:'Space Mono';font-size:1.1em;color:{elo_color}">{elo_label}</div>
                <div style="font-size:.8em;color:#aaa">{elo_diff}</div>
                <div style="font-size:.75em;color:#666">ELO probs: {pred.get('elo_home_p','?')}% / {pred.get('elo_draw_p','?')}% / {pred.get('elo_away_p','?')}%</div>
            </div>""", unsafe_allow_html=True)
        ...
        with src3:
            ...
            st.markdown(f"""<div class="metric-card">
                <div style="font-size:.75em;color:{fat_color}">😴 Fatiga por Calendario</div>
                <div style="font-family:'Space Mono';font-size:1.1em;color:{fat_color}">×{hf:.3f} / ×{af:.3f}</div>
                <div style="font-size:.8em;color:#aaa">{ht}: {hg7} pj en 7d · {at}: {ag7} pj en 7d</div>
                <div style="font-size:.75em;color:#666">1.0 = sin fatiga</div>
            </div>""", unsafe_allow_html=True)

        with src4:
            mh = data.get("market_h"); md = data.get("market_d"); ma = data.get("market_a")
            mkt_color = "#00ff88" if mh else "#666"
            mkt_label = f"{mh}% / {md}% / {ma}%" if mh else "Sin momios importados"
            blend_txt = "Blend 65/35 activo" if pred.get("market_blend") else "Sin blend"
            st.markdown(f"""<div class="metric-card">
                <div style="font-size:.75em;color:{mkt_color}">📈 Line Movement</div>
                <div style="font-family:'Space Mono';font-size:1.1em;color:{mkt_color}">{blend_txt}</div>
                <div style="font-size:.8em;color:#aaa">Mercado: {mkt_label}</div>
                <div style="font-size:.75em;color:#666">Importa momios en Tab 📡 primero</div>
            </div>""", unsafe_allow_html=True)
