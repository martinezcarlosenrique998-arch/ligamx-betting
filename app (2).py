...
with tab1:
    ...
    # Ver cuántos requests quedan y estado de la cuenta
    r_status = api_football("status", {}, football_api_key)
    st.write("📊 Estado cuenta:", r_status.get("response", {}))

    # Buscar Liga MX con distintos IDs conocidos
    for lid in [262, 269, 11]:
        rx = api_football("fixtures", {"league": lid, "season": 2025, "next": 3}, football_api_key)
        count = len(rx.get("response", []))
        errors = rx.get("errors", {})
        st.write(f"League ID {lid} season 2025: {count} fixtures | errores: {errors}")

    # Intentar sin filtro de liga — solo próximos fixtures
    r_next = api_football("fixtures", {"next": 5, "timezone": "America/Mexico_City"}, football_api_key)
    st.write("Fixtures próximos sin filtro:", len(r_next.get("response", [])), "encontrados")
    if r_next.get("response"):
        for f in r_next["response"][:3]:
            st.write(f"  → {f['teams']['home']['name']} vs {f['teams']['away']['name']} | league: {f['league']['name']} (id:{f['league']['id']})")
    st.stop()
    # FIN DIAGNÓSTICO
