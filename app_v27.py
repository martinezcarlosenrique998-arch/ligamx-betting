...
def get_live_odds(api_key, sport_key, markets="h2h,totals,btts"):
    """Fetcha momios en vivo. Por defecto pide h2h + totals + btts en una sola llamada.
    The Odds API plan gratuito: 500 requests/mes. h2h,totals,btts = 1 request."""
...
def extract_market_odds(bookmakers, market_key):
    """Extrae el mejor momio disponible para un mercado específico de una lista de bookmakers.
    Retorna dict con outcomes o None si no hay datos."""
    for bm in bookmakers:
        for mkt in bm.get("markets", []):
            if mkt["key"] == market_key:
                return {"bookmaker": bm["title"], "outcomes": mkt["outcomes"]}
    return None

def best_odds_for_market(live_data, home_name, away_name, market_key):
    """Encuentra el mejor momio disponible para un mercado en todos los bookmakers.
    Para totals: retorna {over_25: X, under_25: Y} con el mejor momio de cada lado.
    Para btts: retorna {yes: X, no: Y}."""
        ...
        for mkt in bm.get("markets", []):
            if mkt["key"] == market_key:
                for outcome in mkt["outcomes"]:
                    ...
                    # totals: Over/Under con point (2.5, 3.5, etc.)
                    if market_key == "totals":
                        point = outcome.get("point", desc)
                        key   = f"{name}_{point}"
                        # Guardar el mejor (mayor) momio para overs, menor para unders
...
def calc_all_predictions(home_form, away_form, home_gf, away_gf,
    ...
    # Blend Over 2.5 con historial real de equipos (25% Poisson, 75% historial)
    # v5 backtest: bucket 50% Over 2.5 subestimado +9.3% (PL) y +9.9% (BL1)
    # Reducir peso Poisson de 40% a 25% corrige sin romper el bucket 60% que ya estaba bien
    ...
    # Blend: 0% Poisson + 100% historial real
    # v5 backtest: Poisson subestima BTTS estructuralmente (pred 41% vs real 52% en bucket 40%)
    # Eliminar Poisson del blend mejora calibración y sharpe (1.47 en PD)
    btts_yes  = btts_hist
    ...
    if home_team_id and away_team_id:
        ...
        if st.button("🔮 ANALIZAR AUTOMÁTICAMENTE", use_container_width=True, type="primary"):
            ...
            st.session_state.update({
                ...
                "last_pred": pred,  # para paper trading
            })
            # Limpiar odds cacheadas del partido anterior
            st.session_state.pop("live_totals", None)
            st.session_state.pop("live_btts", None)
...
with tab2:
            ...
            if isinstance(live_data, dict) and live_data.get("error_code"):
                ...
                if "home_team" in st.session_state:
                    ...
                    if matched:
                        ...
                        # También fetchear totals y btts automáticamente
                        totals_data = best_odds_for_market(live_data, st.session_state["home_team"], st.session_state["away_team"], "totals")
                        btts_data   = best_odds_for_market(live_data, st.session_state["home_team"], st.session_state["away_team"], "btts")
                        if totals_data:
                            st.session_state["live_totals"] = totals_data
                        if btts_data:
                            st.session_state["live_btts"] = btts_data
                        n_markets = 1 + (1 if totals_data else 0) + (1 if btts_data else 0)
                        st.success(f"🎯 {len(matched)} casas · {n_markets} mercados importados para **{ht2} vs {at2}**")
                        if totals_data:
                            over25 = totals_data.get("Over_2.5", totals_data.get("Over_2", "—"))
                            under25 = totals_data.get("Under_2.5", totals_data.get("Under_2", "—"))
                            st.caption(f"Over/Under 2.5: Over {'+' if isinstance(over25,int) and over25>0 else ''}{over25} · Under {'+' if isinstance(under25,int) and under25>0 else ''}{under25}")
                        if btts_data:
                            st.caption(f"BTTS: Sí {'+' if isinstance(btts_data.get('yes',0),int) and btts_data.get('yes',0)>0 else ''}{btts_data.get('yes','—')} · No {'+' if isinstance(btts_data.get('no',0),int) and btts_data.get('no',0)>0 else ''}{btts_data.get('no','—')}")
                    else:
    ...
    # Pre-poblar con odds reales si están disponibles
    _live_totals = st.session_state.get("live_totals", {})
    _live_btts   = st.session_state.get("live_btts", {})
    _has_live    = bool(_live_totals or _live_btts)
    if _has_live:
        st.info("📡 Odds reales importadas — campos pre-poblados con mejores momios del mercado")

    with st.expander("Over/Under — Ingresar momios", expanded=_has_live):
        ou1,ou2,ou3 = st.columns(3)
        with ou1:
            over15_odds  = st.number_input("Over 1.5",  value=float(_live_totals.get("Over_1.5",  _live_totals.get("Over_1", -200))), step=5.0, key="ou_o15")
            under15_odds = st.number_input("Under 1.5", value=float(_live_totals.get("Under_1.5", _live_totals.get("Under_1", 160))),  step=5.0, key="ou_u15")
        with ou2:
            over25_odds  = st.number_input("Over 2.5",  value=float(_live_totals.get("Over_2.5",  _live_totals.get("Over_2", -110))),  step=5.0, key="ou_o25")
            under25_odds = st.number_input("Under 2.5", value=float(_live_totals.get("Under_2.5", _live_totals.get("Under_2", -110))), step=5.0, key="ou_u25")
        with ou3:
            over35_odds  = st.number_input("Over 3.5",  value=float(_live_totals.get("Over_3.5",  _live_totals.get("Over_3", 160))),   step=5.0, key="ou_o35")
            under35_odds = st.number_input("Under 3.5", value=float(_live_totals.get("Under_3.5", _live_totals.get("Under_3", -200))), step=5.0, key="ou_u35")

    with st.expander("BTTS — Ingresar momios", expanded=_has_live):
        b1,b2 = st.columns(2)
        with b1: btts_yes_odds = st.number_input("Ambos anotan - SÍ", value=float(_live_btts.get("yes", -120)), step=5.0, key="btts_y")
        with b2: btts_no_odds  = st.number_input("Ambos anotan - NO", value=float(_live_btts.get("no",  -110)), step=5.0, key="btts_n")
...
with tab4:
    ...
    st.caption("Registra tus apuestas sin dinero real. Trackea tu ROI y aprende qué mercados funcionan.")
    ...
    def pt_load():
        try:
            r = __import__("streamlit").session_state.get("_pt_cache")
            if r:
                return r
        except:
            pass
        try:
            ...
            res = _st.session_state.get("pt_data", [])
            return res
        except:
            return []
    ...
    def pt_save(trades):
        import streamlit as _st
        _st.session_state["pt_data"] = trades
    ...
    # Pre-poblar desde análisis activo si existe
    _pred_live  = st.session_state.get("last_pred", {})
    _match_live = f"{st.session_state.get('home_team','')} vs {st.session_state.get('away_team','')}" if st.session_state.get("home_team") else ""

    with st.form("nueva_apuesta", clear_on_submit=True):
        fc1, fc2 = st.columns(2)
        with fc1:
            pt_partido   = st.text_input("Partido", value=_match_live, placeholder="Ej: Bayern vs Dortmund")
        ...
        pt_notas = st.text_area("Notas / razonamiento", placeholder="¿Por qué esta apuesta tiene valor?", height=80)

        submitted = st.form_submit_button("💾 Guardar apuesta pendiente", use_container_width=True, type="primary")
        if submitted and pt_partido:
            def _amer_to_dec(a):
                a = float(a)
                return round((a/100)+1 if a > 0 else (100/abs(a))+1, 4)
            dec  = _amer_to_dec(pt_momio)
            ev   = round((pt_prob_mod/100 * dec) - 1, 4)
            kelly_puro = max(0, (pt_prob_mod/100 - (1-(pt_prob_mod/100))/(dec-1)))
            kelly_adj  = round(min(kelly_puro * 0.25, 0.10) * 100, 2)
            new_trade = {
                "id":        len(trades)+1,
                "fecha":     str(pt_fecha),
                "partido":   pt_partido,
                ...
                "resultado": "PENDIENTE",
                "pl":        0.0,
                "registered": _dt.now().strftime("%Y-%m-%d %H:%M"),
            }
            trades.append(new_trade)
            pt_save(trades)
            st.success(f"✅ Apuesta #{new_trade['id']} guardada — {pt_partido} · {pt_mercado} · EV {ev*100:+.1f}%")
            st.rerun()
    ...
    if pendientes:
        ...
        for t in pendientes:
            dec_o = t.get("dec_odds", 1.91)
            with st.expander(f"#{t['id']} — {t['partido']} · {t['mercado']} · {t['fecha']}"):
                rc1, rc2, rc3 = st.columns(3)
                with rc1:
                    st.metric("Stake", f"${t['stake']:.2f}")
                with rc2:
                    st.metric("Momio", f"{'+' if t['momio']>0 else ''}{int(t['momio'])}")
                with rc3:
                    st.metric("EV modelo", f"{t['ev']:+.1f}%")
                rb1, rb2 = st.columns(2)
                with rb1:
                    if st.button(f"✅ GANÓ", key=f"win_{t['id']}", use_container_width=True):
                        pl = round(t["stake"] * (dec_o - 1), 2)
                        for tr in trades:
                            if tr["id"] == t["id"]:
                                tr["resultado"] = "GANÓ"; tr["pl"] = pl
                        pt_save(trades); st.rerun()
                with rb2:
                    if st.button(f"❌ PERDIÓ", key=f"loss_{t['id']}", use_container_width=True):
                        for tr in trades:
                            if tr["id"] == t["id"]:
    ...
    if cerradas:
        ...
        total_roi    = round(total_pl / total_stake * 100, 1) if total_stake else 0
        wins         = sum(1 for t in cerradas if t["resultado"] == "GANÓ")
        win_rate     = round(wins / len(cerradas) * 100, 1) if cerradas else 0
        avg_ev       = round(sum(t["ev"] for t in cerradas) / len(cerradas), 1) if cerradas else 0

        dm1,dm2,dm3,dm4,dm5 = st.columns(5)
        with dm1: st.metric("Apuestas", len(cerradas))
        with dm2: st.metric("Win Rate", f"{win_rate}%")
        with dm3: st.metric("ROI", f"{total_roi:+.1f}%", delta=f"${total_pl:+.2f}")
        with dm4: st.metric("Stake total", f"${total_stake:.2f}")
        with dm5: st.metric("EV promedio", f"{avg_ev:+.1f}%")

        # ROI por mercado
        mercados_stats = {}
        for t in cerradas:
            m = t["mercado"]
            if m not in mercados_stats:
                mercados_stats[m] = {"n": 0, "stake": 0, "pl": 0, "wins": 0}
            mercados_stats[m]["n"] += 1
            mercados_stats[m]["stake"] += t["stake"]
            mercados_stats[m]["pl"] += t["pl"]
            if t["resultado"] == "GANÓ":
                mercados_stats[m]["wins"] += 1

        st.markdown("**ROI por mercado:**")
        mkt_rows = []
        for m, s in sorted(mercados_stats.items(), key=lambda x: -x[1]["pl"]):
            roi_m = round(s["pl"]/s["stake"]*100,1) if s["stake"] else 0
            wr_m  = round(s["wins"]/s["n"]*100,1) if s["n"] else 0
            mkt_rows.append({"Mercado": m, "N": s["n"], "Win%": f"{wr_m}%",
        ...
        for t in reversed(cerradas[-20:]):
            hist_rows.append({
                "Fecha": t["fecha"], "Partido": t["partido"][:30],
                "Liga": t["liga"], "Mercado": t["mercado"],
                "Momio": f"{'+' if t['momio']>0 else ''}{int(t['momio'])}",
                "EV mod.": f"{t['ev']:+.1f}%", "Stake": f"${t['stake']:.2f}",
                "Resultado": t["resultado"], "P&L": f"${t['pl']:+.2f}"
            })
        ...
        if len(cerradas) >= 3:
            bk_hist = []
            bk_current = cerradas[0]["bankroll"]
            for t in cerradas:
                bk_current += t["pl"]
                bk_hist.append({"#": t["id"], "Bankroll": round(bk_current, 2)})
            st.markdown("**Evolución del bankroll:**")
            st.line_chart(pd.DataFrame(bk_hist).set_index("#"))
        ...
        st.info("Aún no hay apuestas registradas. Usa el formulario de arriba para empezar tu paper trading.")
