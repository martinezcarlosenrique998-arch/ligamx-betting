        ...
        if not sc_selected_leagues:
            ...
            if st.button("🚀 ESCANEAR TODAS LAS LIGAS", type="primary", use_container_width=True, key="sc_run"):
                ...
                if sc_all_results:
                    ...
                    # Guardar en session_state para paper trading y calculadora
                    ...
                    st.caption("Ordenadas por EV. Haz clic en **→ Calculadora** para analizar a largo plazo.")

                    # ── Tarjetas por apuesta ──
                    for sc_idx, row in sc_df.iterrows():
                        ev_color  = "#00ff88" if row["EV%"] >= 10 else ("#ffcc00" if row["EV%"] >= 5 else "#ff6b6b")
                        casa_name = row.get("Casa", "—")
                        # Buscar URL de la casa
                        _casa_url = None
                        ...
                        with st.container():
                            ...
                            with cc3:
                                st.markdown(f'<span style="color:{ev_color};font-size:1.2em;font-weight:bold">EV {row["EV%"]:+.1f}%</span>', unsafe_allow_html=True)
                                st.caption(f"Momio: **{row['Momio']}**")
                            with cc4:
                                st.caption(f"Kelly: {row['Kelly%']}%")
                                st.caption(f"Stake: ${row['Stake$']:.2f}")
                            with cc5:
                                if st.button("→ Calc EV", key=f"sc_calc_{sc_idx}", help="Enviar a Calculadora EV"):
                                    # Precargar la calculadora con estos datos
                                    try:
                                        _mom_raw = float(str(row["Momio"]).replace("+", ""))
                                    except Exception:
                                        _mom_raw = -110.0
                                    st.session_state["calc_prefill"] = {
                                        "partido":   row["Partido"],
                                        "mercado":   row["Mercado"],
                                        "momio":     _mom_raw,
                                        "my_prob":   row.get("_my_prob", row.get("Modelo%", 50)),
                                        "mkt_type":  row.get("_mkt_type", "Over/Under"),
                                        "casa":      casa_name,
                                    }
                                    st.toast(f"✅ Cargado en Calculadora EV — ve a la tab 🧮")
                        st.divider()

                    if sc_total_skipped:
                        st.caption(f"ℹ️ {sc_total_skipped} partidos omitidos (sin datos suficientes o sin odds)")
